    ...
    try {
        ...
        require('https').get(fileUrl, (res) => {
            res.pipe(unzipper.Extract({ path: instancePath })).on('close', async () => {
                ...
                exec('npm install --omit=dev', { cwd: instancePath }, (error) => {
                    if (error) io.emit("log-" + botId, "⚠️ Nota: Erro no install ou package.json não encontrado.\n");
                    
                    io.emit("log-" + botId, "🚀 Executando npm start...\n");
                    ...
                    bot.sendMessage(chatId, "✅ Bot **" + botId + "** online!\n🔗 [Terminal](" + DOMAIN + "/terminal/" + botId + ")", { parse_mode: 'Markdown' });
                    aresBanner();
                });
            });
        });
    } catch (e) {
        bot.sendMessage(chatId, "❌ Erro ao processar bot: " + e.message);
    }
}
...
bot.onText(/\/start/, (msg) => {
    bot.sendMessage(msg.chat.id, "Host Ares Nexus Online. Envie o arquivo .zip.");
    aresBanner();
});
