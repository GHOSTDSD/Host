...
bot.sendMessage(msg.chat.id,`Bot ${name} criado`)
...
if(action==="menu_new"){
bot.sendMessage(query.message.chat.id,"Envie o ZIP")
}

else if(action==="menu_list"){

const buttons = fs.readdirSync(BASE_PATH).map(f=>[
{text:`${activeBots[f]?"ONLINE":"OFFLINE"} ${f}`,callback_data:`manage:${f}`}
])

bot.editMessageText("Seus bots",{
...
[{text:isRunning?"Parar":"Iniciar",callback_data:`${isRunning?"stop":"restart"}:${id}`}],
[{text:"Voltar",callback_data:"menu_list"}]
]
}})
}
